...
| O / P | 降速/提速(WASD 驱动力实时步进 350,终端速度约 ±98 cm/s 每按一次,范围 600–7200) |
| F | 交互:拾取物品/钥匙、对锁住的门看提示(重力开关通常滚过即触发) |
| 空格 | 拾取提示占满屏幕中央时按空格继续(输入临时锁住,球自然滑停) |
| R | 重置(门回锁、拾取物/钥匙复原、锁屏解除) |

> 相机说明:测试关卡使用**导轨相机**(相机套在 `GSCameraRail` 轨道上跟随,防晕设计)。**没有导轨的关卡用第三人称回退相机**:滚转同样锁世界竖直——**G 翻转后画面永不颠倒**,只随球平移并做小幅瞄准微调(地板态球在画面下三分之一,天花板态在上三分之一,抬升/瞄准高度都在球 Pawn 细节面板 `GravityShift|Camera` 可调);转动平滑按角距自适应,鼠标微调即时。
...
**GravityShift 和 Blockout Tools 都是仓库内插件**(在 `Plugins/` 目录下,克隆即得),**不是 Epic 市场插件,不用去 Fab 下载**;所谓"安装"= 克隆仓库 + 首次打开编译一次。换新电脑/新同学入伙照上面三步走即可。
