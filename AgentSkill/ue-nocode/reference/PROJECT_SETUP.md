...
## 不开编辑器盘点 umap(2026-09-13 第二关轮定型)

> 队友刚推了新地图/大改关卡,先离线判"改了哪张图、加减了什么、哪组是哪一关"。能开编辑器时优先直接进编辑器盘点(见本节第 4 条),离线只是临时手段。

1. **演员清单(可靠)**:正则扫二进制 `PersistentLevel\.([A-Za-z0-9_]+),` 抽全量 actor 名,新旧两版各自成集合做差 = 精确 +N/-M 清单(本轮 36→64、+29/-1;两种独立抽法互证一致)。
2. **别做"全文件可打印字符串集合差分"**:umap 里混着大段序列化 blob,任何字节变化都让整条 blob 进"新增",输出全是噪声;要么用上一条正则,要么限串长+关键词过滤。
3. **位置能从 blob 解(double 三元组),但必须用已知坐标校验**:本轮拿队友文档里的方块坐标 (860,2470,300)/(1036,-955,900)/(600,-2000,100) 全部对上才敢用;偏移错位一步就全错——对不上已知值就**整批弃用**,别半信半疑。
4. **权威分组只能进编辑器拿**:`actor.get_folder_path()`(如 P1/P2 文件夹=第一关/第二关)+ `get_actor_label()`(编号标签);多楼层结构用 `line_trace_single` 在竖井旁逐点向下扫,即可画出地面/夹层/顶面分层坐标。
